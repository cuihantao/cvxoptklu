...
KLU_EXTRA_COMPILE_ARGS = ["-Wno-unknown-pragmas"] if not MSVC else []
...
if not SUITESPARSE_SRC_DIR:
    ...
    klu = Extension('klu',
                    ...
                    define_macros = MACROS + [('NTIMER', '1'), ('NCHOLMOD', '1')],
                    libraries = LAPACK_LIB + BLAS_LIB,
                    extra_compile_args = KLU_EXTRA_COMPILE_ARGS,
                    extra_link_args = BLAS_EXTRA_LINK_ARGS,
                    sources = ['src/C/klu.c' ] +
                        [SUITESPARSE_SRC_DIR + '/SuiteSparse_config/SuiteSparse_config.c'] +
...
setup (name = 'cvxoptklu',
    description = 'CVXOPT KLU Add-on',
...
language.''',
    ...
    ext_package = "cvxoptklu",
    ext_modules = extmods,
    package_dir = {"cvxoptklu": "src/python"},
    packages = ["cvxoptklu"],
